...
from Functions import Data, RMS, Create_dir

np.random.seed(2)

PNG_path, PDF_path = Create_dir('GD')
...
plt.xlabel(r'$\gamma$')
plt.ylabel('$N$')
plt.savefig(PNG_path / 'GD_RMS_gamma_iter')
plt.savefig(PDF_path / 'GD_RMS_gamma_iter.pdf')
...
plt.xlabel(r'$\lambda$')
plt.ylabel('$N$')
plt.savefig(PNG_path / 'GD_RMS_lambda_iter')
plt.savefig(PDF_path / 'GD_RMS_lambda_iter.pdf')
plt.show()